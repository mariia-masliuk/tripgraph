...
class Gallery {
  ...
  filterItems() {
    ...
    let result = [];
    if (t1 < t2) {
      result = this.galleryItems.filter(function (el) {
        return el.ms_start >= t1 && el.ms_start <= t2;
      });
    } else {
      let dec = new Date("2022-12-31").toISOString();
      dec = Date.parse(dec);
      let jan = new Date("2022-01-01").toISOString();
      jan = Date.parse(jan);
      result = this.galleryItems.filter(function (el) {
        return (
          (el.ms_start >= t1 && el.ms_start <= dec) ||
          (el.ms_start >= jan && el.ms_start <= t2)
        );
      });
    }
